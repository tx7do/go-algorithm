...
func TestTernarySearchSingle(t *testing.T) {
	assert.Equal(t, TernarySearch(IntSlice{1, 10, 20, 30, 40, 50, 60}, 10), 1)
	//assert.Equal(t, TernarySearch(IntSlice{1, 10, 20, 30, 40, 50, 60}, 60), 6)
}
...
func TestTernarySearch(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := TernarySearch(tt.args.array, tt.args.target); got != tt.want {
